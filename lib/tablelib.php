...
class flexible_table {
    ...
    function get_sql_where() {
        ...
        if(!empty($this->sess->i_first)) {
            $conditions[] = "firstname $LIKE '{$this->sess->i_first}%'";
        }
        if(!empty($this->sess->i_last)) {
            $conditions[] = "lastname $LIKE '{$this->sess->i_last}%'";
        }
...
class table_sql extends flexible_table{
    ...
    function out($pagesize, $useinitialsbar, $downloadhelpbutton=''){
        if (!$this->columns){
            $onerow = get_record_sql("SELECT {$this->sql->fields} FROM {$this->sql->from} WHERE {$this->sql->where}");
